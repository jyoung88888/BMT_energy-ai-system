...
class SolarPowerService:
    ...
    async def aggregate_and_insert(self, target_date: str) -> Dict[str, Any]:
        ...
        try:
            # 현재 시간 (reg_dt)
            reg_dt = datetime.now()
            logger.info(f"📊 [Solar Power] 등록 시간 (reg_dt): {reg_dt}")

            # 날짜 범위 계산 (인덱스 활용을 위해 범위 조건 사용)
            # target_date (YYYY-MM-DD) 기준으로 하루 범위 설정
            params = [target_date, target_date, target_date, target_date, target_date, reg_dt]
            ...
            # tmn: MIN (0이 아닌 값 중)
            # ics: SUM
            # reg_dt: 현재 시간 (데이터 적재 시각)
            # 인덱스 활용을 위해 DATE() 함수 대신 범위 조건 사용
            query = f"""
            INSERT INTO {self.ai_solar_power_table}
                (ymdhms, tmn, tmx, ics, pre_pwr_generation, today_generation, accum_generation, reg_dt)
            SELECT * FROM (
                SELECT
                    %s as ymdhms,
                    wi_agg.tmn,
            ...
            async with self.db.get_async_connection() as connection:
                def _execute():
                    ...
                    try:
                        ...
                        connection.commit()
                        logger.info(f"✅ [Solar Power] UPSERT 실행 완료")
                        return True
                    finally:
                        cursor.close()
            ...
            logger.info(f"✅ [Solar Power] 데이터 집계 및 적재 완료 | reg_dt: {reg_dt}")

            return {
                "success": True,
                "target_date": target_date,
                "message": f"{target_date} 날짜의 Solar Power 데이터 UPSERT 완료"
            }
