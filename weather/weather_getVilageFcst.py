...
def get_error_description(code):
    ...
    return descriptions.get(code, f'알 수 없는 에러 (코드: {code})')
...
def request_with_retry(url, params, max_retries=MAX_RETRIES, timeout=20):
    ...
    성공/실패 구분:
        - 성공: HTTP 200 + resultCode '00' → 데이터 반환
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 재시도 불가 에러 (서비스키 만료, 잘못된 파라미터 등)
            if result_code in NO_RETRY_CODES:
                logger.error(f"[API 영구실패] 코드: {result_code}, 메시지: {result_msg}")
                logger.error(f"[API 영구실패] 원인: {get_error_description(result_code)}")
                return None
...
def fetch_vilagefcst_data(
    ...
    try:
        # API 호출 (재시도 포함)
        data = request_with_retry(API_URL, params, timeout=timeout)
